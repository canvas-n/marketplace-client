...
import { useCancelSellNft } from "@/react-query/hooks/nft";
const Web3 = require("web3");
const ethers = require("ethers");
const OrderCancelModal = ({ setClose, contractAddr, tokenId, refetch }) => {
  const router = useRouter();
  const { user } = useUser();
  const { cancelSellNft } = useCancelSellNft();
  ...
  const startCancel = async () => {
    const { success, data, msg } = await cancelSellNft({
    ...
    >
      <div className="bg-white px-[20px] sm:px-[40px] pt-[50px] pb-[33px] sm:pb-[50px] w-[95%] sm:w-[500px] min-h-[230px] relative">
        <>
          ...
          <div
            className={`w-full text-center text-[16px] pt-[50px] ${pre_400.className}`}
          >
            <>
              <p>판매를 취소하시겠습니까?</p>
            </>
